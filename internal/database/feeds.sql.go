...
func (q *Queries) DeleteUserFeed(ctx context.Context, userID uuid.UUID) error {
	_, err := q.db.ExecContext(ctx, deleteUserFeed, userID)
	return err
}
...
const getUserWhoMadeFeed = `-- name: GetUserWhoMadeFeed :one
SELECT user_id FROM feeds
WHERE url = $1
`

func (q *Queries) GetUserWhoMadeFeed(ctx context.Context, url string) (uuid.UUID, error) {
	row := q.db.QueryRowContext(ctx, getUserWhoMadeFeed, url)
